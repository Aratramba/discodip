...
/**
 * Loop over all components and find fragments to be rendered
 * alongside main examples
 */

for (let i = 0, l = components.length; i < l; ++i) {
  if (components[i].fragments && components[i].fragments.length) {
    components = components.concat(components[i].fragments);
  }
}
...
function render(q) {
  ...
  fs.writeFile(`${item.file}.html`, item.html, (err) => {
    ...
    if (typeof options.prerender === "object" && options.prerender !== null) {
      ...
      puppet.send({
        url: `http://localhost:${options.prerender.port}/${options.prerender.path}/${item.slug}.html`,
      });
